...
result_img = resize(result_img,(360, 480, 3))
...
give_color_to_seg_img(preds[0,:,:],12)

    img_is = np.asarray(X_test[0,:,:])#cv2.imread(dir_img + fnm + ".png")
    imclass = np.argmax(preds, axis=3)[0,:,:]
    seg_img = give_color_to_seg_img(imclass,12)
